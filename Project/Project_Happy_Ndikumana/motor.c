...
void forward(uint16_t distance_cm, int sensorDistance, uint8_t mode)
{
    ...
    if(mode == 1) //forward with sensor stop
    {
        initTimer0();
        int distance = calculateStopDistance();

        while((count0 < idealCounts - 1 && count1 < idealCounts - 1) && distance > sensorDistance)
        {
            setPwmDutyCycle(1, 0, dutyCycle);
            setPwmDutyCycle(0, dutyCycle, 0);
...
//                sprintf(out,"sensorDis = %d, distance = %d\n",sensorDistance, distance);
//                putsUart0(out);
            }
        }
    }
...
void reverse(uint16_t distance_cm, int sensorDistance, uint8_t mode)
{
    ...
    if(mode == 1)// reverse stop according to sensor
    {
        initTimer0();
        int distance = calculateStopDistance();
